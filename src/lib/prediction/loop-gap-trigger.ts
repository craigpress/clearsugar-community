...
//
// A low alert fires (returns an AdvisoryAction) iff ALL hold (Python `loop_gap`):
...
//   4. Instantaneous severe floor: ROC-projected < 55 fires regardless (still
//      subject to the saturation gate and veto, exactly as the Python pipeline).
//
...
export function evaluateLowTrigger(
  ...
  // 4. (severity) instantaneous severe floor — informs tier; the ROC alert +
  //    gate already passed, so a severe projection just escalates.
  const severeFloor = rocMin < SEVERE_LOW;
  ...
  const evidence = [
    ...
    severe ? `severe floor: projection < ${SEVERE_LOW}` : `below event low ${EVENT_LOW}`,
