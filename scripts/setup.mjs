 ...
 * optional AI insights, optional remote-access configuration, and the
 * optional iOS companion app, then writes `.env` (and, if requested,
 * `docker/docker-compose.yml` and `ios/Config/Server.xcconfig`).
 *
...
const IOS_XCCONFIG_PATH = path.join(REPO_ROOT, "ios", "Config", "Server.xcconfig");
...
// xcconfig treats "//" as a comment even inside values; the standard escape is
// an empty build-setting reference: `https:/$()/host` resolves back to
// `https://host` at build time.
function xcconfigUrl(url) {
  return url.replace("://", ":/$()/");
}

async function stepIosApp(remote, admin) {
  console.log("\n--- iOS companion app (optional) ---");
  console.log(`
ClearSugar includes an iPhone + Apple Watch companion app (in ios/) with
home/lock-screen widgets, watch complications, a Live Activity, and glucose
alerts. You build it yourself with Xcode on a Mac, and it talks only to this
server -- no third-party services.
`);
  const enabled = await askYesNo("Configure the iOS companion app now?", false);
  if (!enabled) {
    console.log("  Skipped. Run this wizard again (or see ios/README.md) if you change your mind.");
    return { enabled: false, bundlePrefix: "" };
  }

  let serverUrl = remote.appUrl;
  if (serverUrl) {
    const useIt = await askYesNo(`Point the app at ${serverUrl}?`, true);
    if (!useIt) serverUrl = "";
  }
  if (!serverUrl) {
    const lanIp = detectLanIp();
    console.log(`
A LAN URL (http://...) only works while the phone is on your home Wi-Fi --
widgets and alerts will NOT update away from home. For that, use Tailscale
Serve or a reverse proxy with HTTPS (see docs/install/INSTALL.md) and re-run
this step with that URL.
`);
    serverUrl = await ask(
      "Server URL the iPhone should use",
      { defaultValue: lanIp ? `http://${lanIp}:3000` : "" },
    );
  }

  console.log(`
The bundle identifier prefix must be unique to you (Apple rejects duplicates
across developer accounts). Use reverse-DNS of something you control, e.g.
com.yourname.clearsugar.
`);
  const bundlePrefix = await ask("Bundle identifier prefix", {
    defaultValue: "com.example.clearsugar",
  });
  const teamId = await ask(
    "Apple Developer Team ID (optional -- you can pick the team in Xcode instead)",
  );

  const content = [
    "// Generated by `npm run setup` -- safe to edit by hand. Not committed to git.",
    "// The $() in the URL is the standard xcconfig escape: \"//\" would otherwise",
    "// start a comment. It resolves to an empty string at build time.",
    `CLEARSUGAR_SERVER_URL = ${serverUrl ? xcconfigUrl(serverUrl) : ""}`,
    `CLEARSUGAR_BUNDLE_PREFIX = ${bundlePrefix}`,
    `DEVELOPMENT_TEAM = ${teamId}`,
    "",
  ].join("\n");
  await mkdir(path.dirname(IOS_XCCONFIG_PATH), { recursive: true });
  await writeFile(IOS_XCCONFIG_PATH, content);
  console.log(`\nWrote ${path.relative(REPO_ROOT, IOS_XCCONFIG_PATH)}`);

  console.log(`
To build (on a Mac -- see ios/README.md for the full walkthrough):
  1. Clone this repo on the Mac and copy ios/Config/Server.xcconfig over
     (it is gitignored, so it does not travel with the repo).
  2. brew install xcodegen && cd ios && xcodegen generate
  3. Open ClearSugar.xcodeproj, select your signing team, build to your iPhone.
  4. In the app, scan the pairing QR code from the web dashboard (or type the
     server URL), then sign in with the admin account you just created` +
    (admin?.username ? ` ("${admin.username}")` : "") + `.
`);
  if (serverUrl) {
    console.log(`The app will talk to: ${serverUrl}`);
  } else {
    console.log("No server URL set -- the app will ask for one on first launch.");
  }

  return { enabled: true, bundlePrefix };
}
...
function buildEnvFile(v) {
  ...
  p(`APNS_BUNDLE_ID=${v.ios.enabled ? v.ios.bundlePrefix : ""}`);
  p(`APNS_SANDBOX=true`);
  p(`DEMO_MODE=false`);
  p("");
  p("# ─── Remote access ──────────────────────────────────────────────────────────");
...
async function main() {
  ...
  const ios = await stepIosApp(remote, admin);
  ...
  if (ios.enabled) {
    console.log(
      `  ${step++}. Build the iOS app on your Mac -- ios/README.md ` +
        "(config already written to ios/Config/Server.xcconfig).",
    );
  }
  if (remote.appUrl) {
